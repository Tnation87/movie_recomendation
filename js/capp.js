...
function fill_rate(star_name, x) {
    ...
    for (i=1; i<=x; i++){
        elem_id = star_name+i;
        document.getElementById(elem_id).style.color = "#2db9b9";
    }
}
...
function getRandomMedia(){
    ...
    for (idx=0; idx<n_movies; idx++) {
        ...
        html_str += '<div class="movie-title"><a href="http://google.com/search?q=';
        html_str += replaced + '">'+ title +'</a></div>' ;
        ...
        html_str += '<input  type="submit" value="Rate" style="color: #FFFFFF;display: block;background-color: #1e7b7b;">';
        html_str += '<br>' + Math.floor((Math.random() * 10000) + 15) + ' ratings';
        html_str += '</div>';
